		...
		for sym in symbols:
			...
			if demangle(sym.name).startswith((
				...
				if 'nn::sf::detail::EmplacedImplHolder<' in demangle(sym.name):
					vtname = 'IpcObj_' + demangle(sym.name).split('nn::sf::detail::EmplacedImplHolder<')[1].split('>')[0]
				elif 'nn::sf::detail::StdSmartPtrHolder<std::__1::unique_ptr<' in demangle(sym.name):
					vtname = 'IpcPtrObj_' + demangle(sym.name).split('nn::sf::detail::StdSmartPtrHolder<std::__1::unique_ptr<')[1].split('>')[0].split(',')[0]
				elif 'nn::sf::UnmanagedServiceObject<' in demangle(sym.name):
					vtname = 'IpcService_' + demangle(sym.name).split('nn::sf::UnmanagedServiceObject<')[1].split('>')[0].split(', ')[1]
